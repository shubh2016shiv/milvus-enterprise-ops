...
from milvus_ops.index_operations.models.parameters import IndexParams
...
class IndexManager:
    ...
    def __init__(
        ...
        config: IndexOperationConfig | None = None,
    ):
        """
    ...
    async def _verify_collection_exists(
        ...
        try:
            ...
            raise BaseCollectionNotFoundError(f"Failed to verify collection existence: {e}") from e

    async def create_index(
        self,
        collection_name: str,
        field_name: str,
        index_type: str | IndexType,
        metric_type: str | MetricType,
        index_params: dict[str, Any] | IndexParams | None = None,
            ...
            async with collection_lock:
                ...
                if field_schema.dtype not in (
                    ...
                    raise IndexParameterError(
                        ...
                        field_name=field_name,
                        parameter_errors={
                            "field_type": f"Not a vector field: {field_schema.dtype}"
                        },
                    )
                ...
                params_dict = (
                    validated_params.to_dict()
                    if isinstance(validated_params, IndexParams)
                ...
                if wait or self._config.enable_timing:
                    self._tracker_registry.register_build(
    ...
    async def _wait_for_index_build(
        self, collection_name: str, field_name: str, timeout: float | None = None
    ) -> None:
        """
        ...
        try:
            ...
            index_info = await self._connection_manager.execute_operation_async(
                lambda alias: self._describe_index_internal(
                    alias=alias, collection_name=collection_name, field_name=field_name
                ),
                timeout=timeout,
            )
            ...
            raise IndexOperationError(f"Failed to describe index: {str(e)}") from e

    def _describe_index_internal(
        self, alias: str, collection_name: str, field_name: str
    ) -> IndexDescription | None:
        """
    ...
    async def _drop_index_internal(self, alias: str, collection_name: str, field_name: str) -> None:
        ...
        try:
            ...
            raise IndexOperationError(f"Failed to drop index: {str(e)}") from e
    ...
    async def get_index_build_progress(
        ...
        try:
            ...
            logger.error(
                f"Failed to get index build progress for {collection_name}.{field_name}: {e}"
            )
            raise IndexOperationError(f"Failed to get build progress: {str(e)}") from e
    ...
    async def list_indexes(
        ...
        try:
            ...
            raise IndexOperationError(f"Failed to list indexes: {str(e)}") from e
    ...
    async def has_index(
        ...
        try:
            ...
            logger.error(
                f"An unexpected error occurred in has_index for {collection_name}.{field_name}: {e}"
            )
