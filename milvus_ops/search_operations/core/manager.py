...
from .search_ops_exceptions import InvalidSearchParametersError, SearchError
...
class SearchManager:
    ...
    async def search(
        ...
        collection_name: str,
        query: str,
        search_params: dict[str, Any] | SearchParams,
    ) -> SearchResult:
        """
        ...
        try:
            ...
            if rerank_config.enabled:
                search_params = self._reranker.get_search_params(
                    config=rerank_config, search_params=search_params
                )
            ...
            if rerank_config.enabled:
                search_result = self._reranker.process_results(
                    results=search_result, config=rerank_config
                )
    ...
    async def _perform_hybrid_search_with_reranking(
        ...
        try:
            ...
            search_params = await self._hybrid_search._prepare_hybrid_search_params(
                query=query, query_vector=query_vector, config=search_config
            )
