...
class QueryOptimizer:
    ...
    def _optimize_for_top_k(self, params: dict[str, Any], top_k: int) -> dict[str, Any]:
        """
    ...
    def _optimize_for_index_type(
        self, params: dict[str, Any], collection_info: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        elif "ANNOY" in index_type and "search_k" not in optimized:
    ...
    def _optimize_for_metric_type(
        self, params: dict[str, Any], metric_type: str | None
    ) -> dict[str, Any]:
        """
    ...
    def suggest_consistency_level(self, top_k: int, timeout: float | None) -> str:
        """
    ...
    def estimate_search_complexity(
        self, config: SemanticSearchConfig, collection_size: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        if config.top_k > 1000:
            complexity["recommendations"].append("Consider reducing top_k for better performance")

        if config.expr and len(config.expr) > 100:
            complexity["recommendations"].append("Complex filter expression may impact performance")
...
class SearchParamsBuilder:
    ...
    def build(
        self, query_vector: list, collection_info: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
        ...
        """
        # Optimize parameters
        optimized_params = self.optimizer.optimize_search_params(self.config, collection_info)

        # Suggest consistency level
        consistency_level = self.optimizer.suggest_consistency_level(
            self.config.top_k, self.config.timeout
        )
