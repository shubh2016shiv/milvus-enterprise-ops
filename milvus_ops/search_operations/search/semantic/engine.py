...
from ...core.search_ops_exceptions import InvalidSearchParametersError, SearchError
...
class SemanticSearch(BaseSearch[SemanticSearchConfig]):
    ...
    def __init__(
        ...
        self.metrics_collector = MetricsCollector(
            max_history=max_metrics_history, metrics_callback=metrics_callback
        )

        # Initialize optimizer
        self.query_optimizer = QueryOptimizer(enable_auto_tuning=enable_query_optimization)
    ...
    async def search(
        ...
        try:
            ...
            query_vector = await self.resilience_manager.execute(
                self._generate_embedding, sanitized_query
            )
