...
class MetricsCollector:
    ...
    def __init__(
        ...
        metrics_callback: Callable[[SearchMetrics], None] | None = None,
    ):
        """
    ...
    async def get_summary(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            successful_metrics = [
                m for m in self._metrics_history if m.status == SearchStatus.SUCCESS
            ]

            if successful_metrics:
                avg_embedding_time = sum(m.embedding_time_ms for m in successful_metrics) / len(
                    successful_metrics
                )
                avg_search_time = sum(m.search_time_ms for m in successful_metrics) / len(
                    successful_metrics
                )
                avg_total_time = sum(m.total_time_ms for m in successful_metrics) / len(
                    successful_metrics
                )
                avg_results = sum(m.results_count for m in successful_metrics) / len(
                    successful_metrics
                )
            else:
