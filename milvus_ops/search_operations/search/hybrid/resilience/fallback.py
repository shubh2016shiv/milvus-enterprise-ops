...
async def handle_fallback(
    ...
    sparse_results: list[dict[str, Any]] | None,
    ...
    if vector_results is None and sparse_results is not None:
        logger.warning(
            f"Vector search failed, using sparse-only results (count: {len(sparse_results)})"
        )
...
def should_attempt_fallback(error: Exception, fallback_enabled: bool) -> bool:
    """
    ...
    async def attempt_fallback(
        ...
        try:
            results, status = await handle_fallback(
                vector_results, sparse_results, self.enable_fallback
            )
            ...
            if status == SearchStatus.DEGRADED:
                ...
                logger.warning(
                    f"Operating in degraded mode - fallback rate: {self.get_fallback_rate():.2%}"
                )
