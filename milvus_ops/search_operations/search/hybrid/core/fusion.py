...
async def fuse_results_weighted(
    ...
    for doc_id, score_dict in scores.items():
        final_scores[doc_id] = (
            score_dict["vector"] * norm_vector_weight + score_dict["sparse"] * norm_sparse_weight
        )
...
def deduplicate_results(
    results: list[dict[str, Any]], id_field: str = "id"
) -> list[dict[str, Any]]:
    """
    ...
    if len(deduplicated) < len(results):
        logger.debug(f"Removed {len(results) - len(deduplicated)} duplicate documents")

    return deduplicated


def normalize_scores(
    results: list[dict[str, Any]], score_field: str = "distance"
) -> list[dict[str, Any]]:
    """
