...
class BM25SparseVectorGenerator:
    ...
    def __init__(self, config: BM25Config = None, enable_caching: bool = True):
        """
    ...
    def _calculate_idf(self, term: str, total_docs: int) -> float:
        ...
        if self.config.idf_smoothing:
            ...
            # Standard IDF
            idf = math.log(total_docs + 1) if doc_freq == 0 else math.log(total_docs / doc_freq)
    ...
    def _calculate_bm25_score(self, term_freq: int, doc_length: int, idf: float) -> float:
        ...
        normalized_length = doc_length / self._avg_doc_length if self._avg_doc_length > 0 else 1.0

        # BM25 formula with saturation
        numerator = term_freq * (self.config.k1 + 1)
        denominator = term_freq + self.config.k1 * (
            1 - self.config.b + self.config.b * normalized_length
        )
    ...
    async def generate(self, text: str, use_cache: bool = True) -> dict[str, Any]:
        """
        ...
        try:
            ...
            indices, values = zip(*sorted_pairs, strict=False) if sorted_pairs else ([], [])

            result = {"indices": list(indices), "values": list(values)}
            ...
            logger.debug(
                f"Generated BM25 sparse vector - tokens: {len(tokens)}, dimensions: {len(indices)}"
            )
