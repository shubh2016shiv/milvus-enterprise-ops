...
def validate_search_params(collection_name: str, query: str, config: HybridSearchConfig) -> None:
    """
    ...
    # Validate timeout
    if config.timeout <= 0:
        raise InvalidSearchParametersError(f"timeout must be positive, got {config.timeout}")
...
def validate_batch_size(batch_size: int, max_batch_size: int = 100) -> None:
    ...
    """
    if batch_size <= 0:
        raise InvalidSearchParametersError(f"Batch size must be positive, got {batch_size}")
