...
from ..core.search_ops_exceptions import InvalidSearchParametersError, ReRankingError
...
class MilvusReRanker:
    ...
    def __init__(
        ...
        metrics_callback: Callable[[ReRankingMetrics], None] | None = None,
    ):
        """
    ...
    def validate_weights(
        ...
        if num_fields is not None and len(weights) != num_fields:
            result.is_valid = False
            result.errors.append(f"Expected {num_fields} weights, got {len(weights)}")
            return result
    ...
    def create_ranker(self, weights: list[float] | None = None, k: int | None = None) -> Any:
        """
        ...
        try:
            if self.method == MilvusReRankingMethod.WEIGHTED:
                ...
                raise InvalidSearchParametersError(f"Unsupported re-ranking method: {self.method}")
    ...
    async def get_search_params(
        ...
        num_vector_fields: int | None = None,
    ) -> dict[str, Any]:
        """
    ...
    async def process_results(
        ...
        config: ReRankingConfig,
        original_results: SearchResult | None = None,
        ...
        try:
            ...
            if original_results and self.enable_metrics:
                metrics.score_changes = self._calculate_score_changes(
                    original_results.hits, results.hits
                )
            ...
            results.search_params["reranking"] = {
                ...
                if hasattr(config.method, "value")
    ...
    async def analyze_reranking_impact(
        self, original_results: SearchResult, reranked_results: SearchResult
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            original_positions = {
                str(r.get("id", r.get("pk", i))): i for i, r in enumerate(original_results.hits)
            }
            reranked_positions = {
                str(r.get("id", r.get("pk", i))): i for i, r in enumerate(reranked_results.hits)
            }
            ...
            analysis["score_changes"] = self._calculate_score_changes(
                original_results.hits, reranked_results.hits
            )
    ...
    async def get_metrics_summary(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            successful = sum(
                1 for m in self._metrics_history if m.status == ReRankingStatus.SUCCESS
            )
            ...
            successful_metrics = [
                m for m in self._metrics_history if m.status == ReRankingStatus.SUCCESS
            ]
            if successful_metrics:
                avg_reranking_time = sum(m.reranking_time_ms for m in successful_metrics) / len(
                    successful_metrics
                )
                avg_total_time = sum(m.total_time_ms for m in successful_metrics) / len(
                    successful_metrics
                )
                avg_input = sum(m.input_count for m in successful_metrics) / len(successful_metrics)
                avg_output = sum(m.output_count for m in successful_metrics) / len(
                    successful_metrics
                )
            else:
    ...
    def get_recommended_weights(self, field_importance: dict[str, float]) -> list[float]:
        """
...
def compare_ranking_methods(
    ...
    try:
        ...
        original_ids = [
            str(r.get("id", r.get("pk", i))) for i, r in enumerate(original_results.hits[:10])
        ]
        weighted_ids = [
            str(r.get("id", r.get("pk", i))) for i, r in enumerate(weighted_results.hits[:10])
        ]
        rrf_ids = [str(r.get("id", r.get("pk", i))) for i, r in enumerate(rrf_results.hits[:10])]

        # Calculate overlaps
        comparison["agreements"]["weighted_vs_rrf"] = len(set(weighted_ids) & set(rrf_ids))
        comparison["agreements"]["original_vs_weighted"] = len(
            set(original_ids) & set(weighted_ids)
        )
...
def create_adaptive_weights(
    ...
    custom_rules: dict[str, dict[str, float]] | None = None,
) -> list[float]:
    """
    ...
    logger.info(
        ...
        f"{dict(zip(field_types, weights, strict=False))}"
    )
...
def calculate_optimal_rrf_k(
    num_searches: int, expected_top_k: int, diversity_preference: float = 0.5
) -> int:
    """
    ...
    Args:
        ...
        diversity_preference: Preference for diversity
            (0.0 = favor precision, 1.0 = favor diversity)
...
class ReRankingStrategy:
    ...
    def __init__(self):
        ...
        self._performance_history: dict[str, list[float]] = {"weighted": [], "rrf": []}

    async def auto_select_method(
        self,
        num_vector_fields: int,
        query_characteristics: dict[str, Any] | None = None,
        ...
        if num_vector_fields == 2:
            ...
            weighted_avg = (
                sum(self._performance_history["weighted"])
                / len(self._performance_history["weighted"])
                if self._performance_history["weighted"]
                else None
            )
            rrf_avg = (
                sum(self._performance_history["rrf"]) / len(self._performance_history["rrf"])
                if self._performance_history["rrf"]
                else None
            )
        ...
        logger.info(f"Auto-selected re-ranking method: {method.value} with params: {params}")
...
class MultiStageReRanker:
    ...
    def __init__(
        ...
        for method, _ in stages:
            ranker = MilvusReRanker(method=method, enable_metrics=enable_metrics)
            self.rankers.append(ranker)

        logger.info(f"MultiStageReRanker initialized with {len(stages)} stages")

    async def rerank(self, results: SearchResult, configs: list[ReRankingConfig]) -> SearchResult:
        """
        ...
        if len(configs) != len(self.stages):
            raise InvalidSearchParametersError(
                ...
                f"number of stages ({len(self.stages)})"
            )

        current_results = results

        for i, (ranker, config) in enumerate(zip(self.rankers, configs, strict=False)):
            logger.info(f"Applying re-ranking stage {i + 1}/{len(self.stages)}")

            current_results = await ranker.process_results(
                results=current_results, config=config, operation_id=f"stage_{i + 1}"
            )
...
def create_text_search_reranker(
    enable_sparse: bool = True, dense_weight: float = 0.6, sparse_weight: float = 0.4
) -> MilvusReRanker:
    """
    ...
    if enable_sparse:
        ...
        return MilvusReRanker(method=MilvusReRankingMethod.RRF, enable_validation=True)


def create_multimodal_reranker(
    modalities: list[str], query_modality: str = "text"
) -> MilvusReRanker:
    """
...
def create_ensemble_reranker(num_models: int, diversity_preference: float = 0.5) -> MilvusReRanker:
    """
    ...
    ranker = MilvusReRanker(
        method=MilvusReRankingMethod.RRF, enable_validation=True, fallback_on_error=True
    )

    logger.info(f"Created ensemble re-ranker for {num_models} models with k={k}")
