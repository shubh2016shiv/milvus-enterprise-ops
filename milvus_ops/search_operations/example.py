...
class SimpleEmbeddingProvider(EmbeddingProvider):
    ...
    async def generate_embeddings(self, texts: list[str]) -> EmbeddingResult:
        ...
        embeddings = [[random.random() for _ in range(self.dimension)] for _ in texts]
...
async def run_example():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Found {len(semantic_results.hits)} results in {semantic_results.took_ms:.2f}ms"
            )
        ...
        try:
            ...
            logger.info(
                f"Found {len(reranked_results.hits)} results in {reranked_results.took_ms:.2f}ms"
            )
        ...
        try:
            ...
            logger.info(
                f"Found {len(hybrid_results.hits)} results in {hybrid_results.took_ms:.2f}ms"
            )
