...
class MilvusCircuitBreaker:
    ...
    def __init__(
        self, config: CircuitBreakerConfig | None = None, name: str = "milvus"
    ):
        """
        ...
        if was_half_open:
            async with self._half_open_lock:
                ...
                logger.debug(
                    ...
                    f"{self.config.max_half_open_requests}"
                )
        ...
        async with self._lock:
            if self._state == CircuitState.OPEN:
                ...
                if self._half_open_requests >= self.config.max_half_open_requests:
                    self._total_fast_failures += 1
                    raise ServerUnavailableError(
                        "Milvus service recovery testing in progress. Please retry in a moment."
                    )
        ...
        async with self._lock:
            if self._state == CircuitState.HALF_OPEN:
                ...
                if self._failure_count > 0:
                    logger.debug(
                        f"Circuit '{self.name}': Resetting failure count after success"
                    )
                    self._failure_count = 0
    ...
    async def _transition_to_open(self):
        ...
        self._state_change_count += 1
        self._last_state_change = time.monotonic()
        # Ensure _last_failure_time is set for recovery timeout calculation
