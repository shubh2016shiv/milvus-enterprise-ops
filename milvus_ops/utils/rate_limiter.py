...
class TokenBucketRateLimiter(RateLimiter):
            ...
            while True:
                ...
                if self.tokens >= tokens:
                    ...
                    if total_wait > 0:
                        ...
                        self._metrics.peak_wait_time = max(self._metrics.peak_wait_time, total_wait)
    ...
    def try_acquire(self, tokens: int = 1) -> bool:
        ...
        self._metrics.total_requests += 1
        self._metrics.total_throttled += 1
        self._metrics.throttle_rate = self._metrics.total_throttled / self._metrics.total_requests
        return False
...
class SlidingWindowRateLimiter(RateLimiter):
    ...
    def __init__(self, max_requests: int, window_seconds: float):
        ...
        logger.debug(
            f"SlidingWindowRateLimiter initialized: {max_requests} requests per {window_seconds}s"
        )
