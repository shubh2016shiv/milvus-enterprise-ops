...
from ..models.entities import BackupMetadata, BackupState, BackupStorageType
...
class MilvusNativeBackupBackend:
    ...
    def _find_milvus_backup_binary(self) -> str | None:
        """
        ...
        try:
            ...
            cmd = ["where", "milvus-backup"] if os.name == "nt" else ["which", "milvus-backup"]
    ...
    async def restore_backup(
        self, backup_name: str, target_collection_name: str | None = None
    ) -> bool:
        """
        ...
        """
        if not self.milvus_backup_bin:
            raise RestoreError("Milvus-Backup tool not found", backup_id=backup_name)

        try:
            logger.info(f"Starting Milvus native restore for backup '{backup_name}'")

            # Build restore command
            cmd = [self.milvus_backup_bin, "restore", "-n", backup_name]
