...
async def example_partition_backup(backup_manager: BackupManager, collection_name: str):
    ...
    try:
        ...
        if result.success:
            logger.info("✓ Partition backup created!")
            logger.info(
                f"  Partitions: {result.metadata.partition_names if result.metadata else 'N/A'}"
            )
            logger.info(f"  Size: {result.size_mb:.2f} MB")
        else:
            logger.error(f"✗ Backup failed: {result.error_message}")
...
async def example_verify_backup(
    backup_manager: BackupManager, backup_id: str, collection_name: str
):
    ...
    try:
        ...
        result = await backup_manager.verify_backup(
            backup_id=backup_id,
            collection_name=collection_name,
            params=VerificationParams(verification_type=VerificationType.CHECKSUM, fail_fast=True),
        )
...
async def example_apply_retention_policy(backup_manager: BackupManager, collection_name: str):
    ...
    try:
        ...
        result = await backup_manager.apply_retention_policy(
            collection_name=collection_name, dry_run=True
        )

        logger.info("Retention policy analysis:")
...
async def example_milvus_native_backup(backup_manager: BackupManager, collection_name: str):
    ...
    try:
        result = await backup_manager.create_backup(
            collection_name=collection_name,
            params=BackupParams(backup_type=BackupType.FULL_COLLECTION, compression_enabled=True),
            storage_type=BackupStorageType.MILVUS_NATIVE,
        )

        if result.success:
            logger.info("✓ Milvus native backup created!")
            logger.info(f"  Backup ID: {result.backup_id}")
        else:
            logger.error(f"✗ Backup failed: {result.error_message}")
...
async def main():
    ...
    try:
        ...
        if backups:
            ...
            # await example_restore_backup(backup_manager, backup_id, collection_name)
