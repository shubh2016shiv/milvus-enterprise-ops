...
class CompressionHandler:
    ...
    def __init__(self, compression_level: int = 6, prefer_zstd: bool = True):
        """
    ...
    async def _compress_file_gzip(self, input_path: Path, output_path: Path) -> None:
        ...
        with (
            ...
            gzip.open(output_path, "wb", compresslevel=self.compression_level) as f_out,
        ):
            while True:
    ...
    def compress_data(self, data: bytes) -> bytes:
        ...
        try:
            ...
            raise ValueError(f"Data compression failed: {e}") from e

    def decompress_data(self, data: bytes, is_zstd: bool | None = None) -> bytes:
        """
        ...
        try:
            ...
            if is_zstd is None:
                ...
                is_zstd = len(data) >= 4 and data[0:4] == b"\x28\xb5\x2f\xfd"
            ...
            raise ValueError(f"Data decompression failed: {e}") from e
    ...
    @staticmethod
    def is_compressed(file_path: str | Path) -> bool:
        """
    ...
    def get_estimated_compressed_size(self, original_size: int) -> int:
        ...
        # zstd typically achieves 2-4x compression, gzip 2-3x
        ratio = 0.4 if self.use_zstd else 0.5  # Assume 40% or 50% of original size
