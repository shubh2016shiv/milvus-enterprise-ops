...
class RetentionPolicyManager:
    ...
    def apply_retention_policy(
        self, backups: list[BackupVersion]
        ...
        for collection_name, collection_backups in backups_by_collection.items():
            keep, delete = self._apply_policy_to_collection(collection_name, collection_backups)
            to_keep.extend(keep)
            to_delete.extend(delete)

        logger.info(f"Retention policy applied: {len(to_keep)} to keep, {len(to_delete)} to delete")

        return to_keep, to_delete

    def _group_by_collection(self, backups: list[BackupVersion]) -> dict[str, list[BackupVersion]]:
        """
    ...
    def _apply_policy_to_collection(
        self, collection_name: str, backups: list[BackupVersion]
    ) -> tuple[list[BackupVersion], list[BackupVersion]]:
        """
        ...
        for i, backup in enumerate(sorted_backups):
            ...
            if should_keep:
                ...
                logger.debug(
                    f"Keeping backup {backup.backup_name} "
                    f"({backup.age_days:.1f} days old): {reason}"
                )
            else:
    ...
    def should_keep_backup(self, backup: BackupVersion, all_backups: list[BackupVersion]) -> bool:
        """
        ...
        collection_backups = [b for b in all_backups if b.collection_name == backup.collection_name]

        to_keep, _ = self._apply_policy_to_collection(backup.collection_name, collection_backups)
