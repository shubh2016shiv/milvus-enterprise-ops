...
from milvus_ops.collection_operations import CollectionManager, CollectionSchema  # noqa: E402
from milvus_ops.connection_management import ConnectionManager  # noqa: E402
from milvus_ops.data_management_operations.core.validator import DataValidator  # noqa: E402
from milvus_ops.data_management_operations.data_ops_config import DataOperationConfig  # noqa: E402
from milvus_ops.data_management_operations.data_ops_exceptions import (  # noqa: E402
...
from milvus_ops.data_management_operations.models.entities import (  # noqa: E402
    BatchOperationResult,
    DeleteResult,
    DocumentBase,
    OperationStatus,
)
from milvus_ops.data_management_operations.utils.retry import (  # noqa: E402
    is_transient_milvus_error,
)
from milvus_ops.data_management_operations.utils.timing import (  # noqa: E402
    BatchTimingResult,
    PerformanceTimer,
    TimingResult,
)
from milvus_ops.milvus_ops_exceptions import (  # noqa: E402
...
class DataManager(Generic[T]):
    ...
    def __init__(
        ...
        config: DataOperationConfig | None = None,
    ):
        """
    ...
    async def _verify_connection_health(self, timeout: float | None = None) -> None:
        """
        ...
        try:
            ...
            if not self._connection_manager.check_server_status():
                ...
                raise ConnectionError(
                    "Milvus server is not responding. Please reinitialize the connection."
                )

        except Exception as e:
            logger.error(f"Connection health check failed: {e}")
            raise ConnectionError(f"Failed to verify Milvus connection health: {e}") from e
    ...
    async def _verify_collection_exists_or_create(
        ...
        try:
            ...
            exists = await self._collection_manager.has_collection(
                collection_name=collection_name, strict=True, timeout=timeout
            )
            ...
            if not exists:
                if auto_create:
                    ...
                    await self._collection_manager.create_collection(
                        collection_name=collection_name, schema=schema, timeout=timeout
                    )

                    # Load the collection
                    await self._collection_manager.load_collection(
                        collection_name=collection_name, wait=True, timeout=timeout
                    )
            ...
            if is_transient_milvus_error(e):
                raise TransientOperationError(f"Transient error checking collection: {e}") from e

            logger.error(f"Error checking collection '{collection_name}': {e}")
            raise CollectionOperationError(f"Failed to verify collection existence: {e}") from e

    async def insert(
        self,
        collection_name: str,
        documents: list[T | dict[str, Any]],
        batch_size: int | None = None,
        timeout: float | None = None,
        validate: bool = True,
        partition_key: str | None = None,
        auto_create_collection: bool = False,
        collection_schema: CollectionSchema | None = None,
    ) -> BatchOperationResult:
        """
        ...
        Args:
            ...
            partition_key: Optional partition key for routing documents to a specific
                partition
            auto_create_collection: Whether to automatically create the collection if it
                doesn't exist
            collection_schema: Schema to use for collection creation (required if
        ...
        if not documents:
            logger.warning(
                f"No documents provided for insertion into collection '{collection_name}'"
            )
            return BatchOperationResult(
                status=OperationStatus.SUCCESS, successful_count=0, failed_count=0
            )
        ...
        async with self._timer.time_operation(
            ...
            except (BaseCollectionNotFoundError, CollectionOperationError):
                error_msg = f"Collection '{collection_name}' does not exist"
                logger.error(f"Error retrieving schema: {error_msg}")
                raise CollectionOperationError(error_msg) from None
            ...
            if validate:
                ...
                if not validation_result.is_valid:
                    ...
                    raise SchemaValidationError(
                        error_msg, validation_errors=validation_result.errors
                    )
                ...
                for i in range(num_batches):
                    ...
                    try:
                        ...
                        batch_result = await self._connection_manager.execute_operation_async(
                            lambda alias, data=insert_data: self._insert_internal(
                                alias=alias,
                                collection_name=collection_name,
                                data=data,
                                partition_key=partition_key,
                        ...
                        logger.error(
                            f"[insert] Error inserting batch {i + 1}/{num_batches} "
                            f"into collection '{collection_name}': {e}"
                        )
            ...
            logger.info(
                f"[insert] Inserted {result.successful_count}/{num_documents} "
                f"documents into collection '{collection_name}'"
            )
            return result
    ...
    def _insert_internal(
        ...
        try:
            ...
            logger.error(
                f"[_insert_internal] Milvus error during insertion in '{collection_name}': {e}"
            )
            raise InsertionError(f"Milvus error during insertion: {e}") from e
        except Exception as e:
            logger.error(
                f"[_insert_internal] Unexpected error during insertion in '{collection_name}': {e}"
            )
            raise InsertionError(f"Unexpected error during insertion: {e}") from e
    ...
    async def upsert(
        ...
        async with self._timer.time_operation(
            ...
            async with collection_lock:
                for i in range(num_batches):
                    ...
                    try:
                        ...
                        batch_result = await self._connection_manager.execute_operation_async(
                            lambda alias, data=upsert_data: self._upsert_internal(
                                alias=alias,
                                collection_name=collection_name,
                                data=data,
                                partition_key=partition_key,
                        ...
                        logger.error(
                            f"[upsert] Error upserting batch {i + 1}/{num_batches} "
                            f"into collection '{collection_name}': {e}"
                        )
                        ...
                        if result.successful_count > 0:
                            ...
                            if isinstance(e, InsertionError | OperationTimeoutError):
                                raise
                            raise InsertionError(f"Failed to upsert documents: {e}") from e
            ...
            logger.info(
                f"[upsert] Upserted {result.successful_count}/{num_documents} "
                f"documents into collection '{collection_name}'"
            )
            return result
    ...
    def _upsert_internal(
        ...
        try:
            ...
            logger.error(
                f"[_upsert_internal] Milvus error during upsert in '{collection_name}': {e}"
            )
            raise InsertionError(f"Milvus error during upsert: {e}") from e
        except Exception as e:
            logger.error(
                f"[_upsert_internal] Unexpected error during upsert in '{collection_name}': {e}"
            )
            raise InsertionError(f"Unexpected error during upsert: {e}") from e
    ...
    async def delete(
        ...
        Args:
            ...
            partition_key: Optional partition key to limit deletion to a specific
                partition
            auto_create_collection: Whether to automatically create the collection if
                it doesn't exist
            collection_schema: Schema to use for collection creation (required if
                auto_create_collection=True)
        ...
        async with self._timer.time_operation(
            ...
            async with collection_lock:
                try:
                    ...
                    logger.info(
                        f"[delete] Deleted {result} documents from collection "
                        f"'{collection_name}' with expression '{expr}'"
                    )

                    return DeleteResult(status=OperationStatus.SUCCESS, deleted_count=result)

                except Exception as e:
                    logger.error(
                        f"Error deleting documents from collection '{collection_name}' "
                    ...
                    raise DeleteOperationError(f"Failed to delete documents: {e}") from e
    ...
    def get_operation_stats(self, operation_name: str) -> BatchTimingResult | None:
        """
    ...
    def _delete_internal(
        ...
        try:
            ...
            raise DeleteOperationError(f"Milvus error during deletion: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error during deletion in '{collection_name}': {e}")
            raise DeleteOperationError(f"Unexpected error during deletion: {e}") from e
