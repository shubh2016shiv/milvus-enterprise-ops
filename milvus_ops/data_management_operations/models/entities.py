...
class DocumentBase(BaseModel):
    ...
    id: int | str | UUID | None = Field(
        None,
        description=(
            "Primary key. If None and auto_id=True in collection, " "Milvus will generate one."
        ),
    )
...
class Document(DocumentBase):
    ...
    vector: list[float] | dict[str, list[float]] | None = Field(
        None,
    ...
    def validate_vector(cls, v):
        ...
        if isinstance(v, list):
            ...
            raise ValueError(
                "Vector must be either a list of numbers or a dictionary of named vectors"
            )
        return v
...
class BatchOperationResult(BaseModel):
    ...
    failed_count: int = 0
    error_messages: dict[int | str, str] = Field(
...
class DeleteResult(BaseModel):
    ...
    timing: TimingResult | None = Field(
        None, description="Performance timing result for the operation."
    )
...
class DataValidationResult(BaseModel):
    ...
    is_valid: bool
    errors: dict[int | str, list[str]] = Field(
        default_factory=dict,
