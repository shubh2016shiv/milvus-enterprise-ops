...
class PerformanceTimer:
    ...
    @asynccontextmanager
    async def time_operation(self, operation_name: str, metadata: dict[str, Any] | None = None):
        """
    ...
    def get_summary(self) -> dict[str, BatchTimingResult]:
        ...
        operation_names = {r.operation_name for r in self._timing_history}
...
def time_operation(operation_name: str, metadata: dict[str, Any] | None = None):
    """
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.debug(
                    f"Operation '{operation_name}' completed in {execution_time * 1000:.2f}ms"
                )
                return result
            except Exception:
                execution_time = time.time() - start_time
                logger.debug(
                    f"Operation '{operation_name}' failed after {execution_time * 1000:.2f}ms"
                )
                raise
