...
from config import MilvusSettings  # noqa: E402

from milvus_ops.collection_operations import (  # noqa: E402
    CollectionManager,
    CollectionSchema,
    DataType,
    FieldSchema,
...
from milvus_ops.data_management_operations import (  # noqa: E402
...
async def create_test_collection(collection_manager: CollectionManager) -> None:
    ...
    # Create collection
    await collection_manager.create_collection(collection_name="test_collection", schema=schema)

    # Load collection
    await collection_manager.load_collection(collection_name="test_collection", wait=True)
...
async def demonstrate_basic_insertion(data_manager: DataManager, collection_name: str):
    ...
    try:
        ...
        result = await data_manager.insert(
            collection_name=collection_name, documents=documents, validate=True
        )
...
async def demonstrate_batched_insertion(data_manager: DataManager, collection_name: str):
    ...
    try:
        ...
        logger.warning("Partial failure in batch insertion")
...
async def demonstrate_upsert_operation(data_manager: DataManager, collection_name: str):
...
async def demonstrate_delete_operation(data_manager: DataManager, collection_name: str):
    ...
    try:
        ...
        result = await data_manager.delete(
            collection_name=collection_name, expr="id >= 100 and id < 110"
        )
...
async def demonstrate_performance_monitoring(data_manager: DataManager):
    ...
    for op_name, stats in summary.items():
        logger.info(
            f"  {op_name}: {stats.total_operations} ops, "
...
async def main():
    ...
    try:
        ...
        logger.info(
            f"Configuration created: batch_size={config.default_batch_size}, "
