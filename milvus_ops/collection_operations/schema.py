...
class DataType(str, Enum):
    ...
    FLOAT = "FLOAT"
    DOUBLE = "DOUBLE"
    VARCHAR = "VARCHAR"  # Canonical name in Milvus (previously called STRING)
...
class IndexParams(BaseModel):
    ...
    params: dict[str, Any] = Field(
        default_factory=dict, description="Additional parameters for the index."
    )
...
class FieldSchema(BaseModel):
    ...
    description: str | None = Field(None, description="An optional description for the field.")
    ...
    auto_id: bool = Field(
        False,
        description="If True, Milvus automatically generates IDs for this primary key field.",
    )
    dim: int | None = Field(None, description="The dimension of the vector field.")
    max_length: int | None = Field(None, description="The maximum length for a VARCHAR field.")
    element_type: DataType | None = Field(
        None, description="The data type of elements in an ARRAY field."
    )
    index_params: IndexParams | None = Field(
        None, description="Optional index parameters for vector fields (metadata only)."
    )
    ...
    def validate_primary_key(cls, model):
        ...
        if model.is_primary:
            ...
            if model.dtype not in allowed_pk_types:
                raise ValueError(
                    f"Primary key must be one of {allowed_pk_types}, got {model.dtype}"
                )
        return model
...
class CollectionSchema(BaseModel):
    ...
    fields: list[FieldSchema] = Field(
        ..., description="A list of fields defining the collection's structure."
    )
    description: str | None = Field(None, description="An optional description for the collection.")
    enable_dynamic_field: bool = Field(
        False,
        description="Allows dynamic fields if set to True. When enabled, fields not defined "
        "in the schema can be inserted.",
    )
    shard_num: int = Field(
        2,
        description="The number of shards to distribute the collection data into. Default is 2. "
        "Higher values improve write throughput for large collections.",
    )
    ...
    def validate_fields(cls, fields):
        ...
        if len(primary_keys) > 1:
            raise ValueError(
                f"Collection has multiple primary keys: {[pk.name for pk in primary_keys]}"
            )
        ...
        if len(partition_keys) > 1:
            raise ValueError(
                f"Collection has multiple partition keys: {[pk.name for pk in partition_keys]}"
            )
    ...
    def compute_hash(self) -> str:
        ...
        for field in self.fields:
            ...
            if field.element_type is not None:
                ...
                functional_field["element_type"] = (
                    field.element_type.value
                    if hasattr(field.element_type, "value")
                    else str(field.element_type)
                )
