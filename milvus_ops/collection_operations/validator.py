...
from .schema import CollectionSchema, DataType
...
class SchemaValidator:
    ...
    def _validate_primary_key(cls, schema: CollectionSchema, errors: list[str]) -> None:
        ...
        if len(primary_keys) > 1:
            errors.append(
                f"Schema defines multiple primary keys: {[pk.name for pk in primary_keys]}"
            )
            return
    ...
    def _validate_field_constraints(cls, schema: CollectionSchema, errors: list[str]) -> None:
        ...
        for field in schema.fields:
            ...
            if field.dtype == DataType.ARRAY and field.element_type is None:
    ...
    async def compare_schemas(
        cls, schema1: CollectionSchema, schema2: CollectionSchema
    ) -> tuple[bool, list[str]]:
        """
        ...
        for name, field1 in fields1.items():
            if name in fields2:
                ...
                dtype1 = cls.normalize_dtype(
                    field1.dtype.value if hasattr(field1.dtype, "value") else str(field1.dtype)
                )
                dtype2 = cls.normalize_dtype(
                    field2.dtype.value if hasattr(field2.dtype, "value") else str(field2.dtype)
                )
                ...
                if (
                    ...
                    and field1.dim != field2.dim
                ):
                    incompatibilities.append(
                        f"Vector field '{name}' has different dimensions: {field1.dim} vs "
                        f"{field2.dim}"
                    )
                ...
                if field1.is_primary != field2.is_primary:
                    incompatibilities.append(
                        f"Field '{name}' has different primary key status: {field1.is_primary} vs "
                        f"{field2.is_primary}"
                    )
