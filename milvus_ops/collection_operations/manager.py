...
from .schema import CollectionSchema, DataType, FieldSchema
...
class CollectionManager:
    ...
    async def create_collection(
        ...
        async with collection_lock:
            ...
            if exists:
                ...
                try:
                    ...
                    (
                        is_compatible,
                        incompatibilities,
                    ) = await SchemaValidator.compare_schemas(schema, existing_schema)
                    ...
                    if is_compatible:
                        ...
                        error_msg = (
                            f"Collection '{collection_name}' already exists with incompatible "
                            f"schema: {', '.join(incompatibilities)}"
                        )
                        logger.error(error_msg)
                        raise SchemaError(error_msg)
            ...
            try:
                ...
                logger.error(
                    f"[create_collection] Failed to create collection '{collection_name}' "
                    f"(alias={getattr(e, 'using', 'unknown')}): {e}"
                )
                ...
                if isinstance(e, SchemaNotReadyException):
                    ...
                    raise SchemaError(f"Schema validation failed during creation: {e}") from e
    ...
    def _create_collection_internal(self, alias: str, **kwargs) -> None:
        ...
        for field_dict in kwargs.get("fields", []):
            ...
            try:
                ...
                raise SchemaError(
                    f"Failed to map data type '{dtype_str}' to Milvus DataType: {e}"
                ) from e

            # Filter field parameters to only those accepted by pymilvus
            filtered_field_dict = {
                k: v for k, v in field_dict.items() if k in self._FIELD_PARAM_WHITELIST
            }
            ...
            if dtype_str == "ARRAY" and "element_type" in filtered_field_dict:
                ...
                try:
                    ...
                    raise SchemaError(
                        f"Failed to map element_type '{element_type_str}' to Milvus DataType: {e}"
                    ) from e
            ...
            try:
                ...
                raise SchemaError(
                    f"Invalid field parameters for "
                    f"'{filtered_field_dict.get('name', 'unknown')}': {e}"
                ) from e

        # Create the pymilvus CollectionSchema
        schema = MilvusCollectionSchema(fields=fields, description=kwargs.get("description", ""))
    ...
    async def has_collection(
        ...
        try:
            ...
            logger.error(
                f"[has_collection] Error checking if collection '{collection_name}' exists "
                f"(alias={getattr(e, 'using', 'unknown')}): {e}"
            )
            if strict:
                ...
                if isinstance(e, ConnectionError | OperationTimeoutError):
                    raise
                raise CollectionError(f"Error checking if collection exists: {e}") from e
            return False
    ...
    async def list_collections(
        self, strict: bool = False, timeout: float | None = None
    ) -> list[str]:
        """
        ...
        try:
            result = await self._connection_manager.execute_operation_async(
                lambda alias: self._list_collections_internal(alias), timeout=timeout
            )
            return await self._ensure_awaited(result)
        except Exception as e:
            logger.error(
                f"[list_collections] Error listing collections "
                f"(alias={getattr(e, 'using', 'unknown')}): {e}"
            )
            if strict:
                # Preserve specific exception types for better error handling
                if isinstance(e, ConnectionError | OperationTimeoutError):
                    raise
                raise CollectionError(f"Error listing collections: {e}") from e
            return []
    ...
    async def describe_collection(
        ...
        try:
            ...
            logger.error(error_msg)
            raise CollectionNotFoundError(error_msg) from None
        except Exception as e:
            logger.error(
                f"[describe_collection] Error describing collection '{collection_name}' "
                f"(alias={getattr(e, 'using', 'unknown')}): {e}"
            )
            ...
            if isinstance(e, SchemaError | SchemaNotReadyException):
                raise SchemaError(f"Schema validation failed during describe: {e}") from e
            raise CollectionError(f"Error describing collection: {e}") from e
    ...
    def _describe_collection_internal(
        self, alias: str, collection_name: str
    ) -> CollectionDescription:
        """
        ...
        for field in milvus_schema.fields:
            ...
            try:
                ...
                if not hasattr(DataType, dtype_name):
                    raise SchemaError(
                        f"Cannot map Milvus data type {milvus_dtype_name} to our DataType enum"
                    )

            except AttributeError as e:
                raise SchemaError(f"Failed to get data type from field {field.name}: {e}") from e
            ...
            if (
                dtype_name == "ARRAY"
                and hasattr(field, "element_type")
                and field.element_type is not None
            ):
                try:
                    ...
                    if not hasattr(DataType, mapped_element_type):
                        raise SchemaError(
                            f"Cannot map Milvus element type {element_dtype_name} to our "
                            f"DataType enum"
                        )
                    ...
                    logger.warning(
                        f"Failed to extract element_type from ARRAY field {field.name}: {e}"
                    )
        ...
        return CollectionDescription(
            ...
            created_at_is_synthetic=created_at_is_synthetic,  # Flag indicating if date is
            # synthesized
        )
        ...
        async with collection_lock:
            ...
            try:
                ...
                try:
                    ...
                    if ignore_index_errors and (
                        "index" in error_str and ("exist" in error_str or "found" in error_str)
                    ):
                ...
                logger.info(
                    f"Waiting for collection '{collection_name}' to load (timeout={timeout}s)..."
                )
                ...
                while True:
                    ...
                    if iteration % 5 == 0:
                        logger.info(
                            f"Load progress for '{collection_name}': state={progress.state.value}, "
                            f"progress={progress.progress:.2%}, iteration={iteration}"
                        )

                    if progress.is_complete:
                        logger.info(
                            f"Collection '{collection_name}' load complete: "
                    ...
                    if timeout and elapsed > timeout:
                        error_msg = (
                            f"Timed out waiting for collection '{collection_name}' to load after "
                            f"{elapsed:.1f}s. Last state: {progress.state.value}, "
                            f"progress: {progress.progress:.2%}"
                        )
                        logger.error(error_msg)
                    ...
                    if iteration > 20 and progress.progress == 0.0:
                        error_msg = (
                            f"Collection '{collection_name}' appears stuck at 0% progress after "
                            f"{iteration} iterations. State: {progress.state.value}. This may "
                            f"indicate the collection is already loaded or there's an issue with "
                        ...
                        raise OperationTimeoutError(f"Collection load appears stuck. {error_msg}")
                ...
                logger.error(
                    f"[load_collection] Error loading collection '{collection_name}' "
                    f"(alias={getattr(e, 'using', 'unknown')}): {e}"
                )
                if isinstance(e, CollectionNotFoundError | OperationTimeoutError):
                    raise
                raise CollectionError(f"Error loading collection: {e}") from e
    ...
    def _load_collection_internal(self, alias: str, collection_name: str) -> None:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to load collection: {e}") from e
    ...
    async def get_load_progress(
        ...
        try:
            ...
            logger.error(
                f"[get_load_progress] Error getting load progress for collection "
                f"'{collection_name}' (alias={getattr(e, 'using', 'unknown')}): {e}"
            )
            if isinstance(e, CollectionNotFoundError | OperationTimeoutError):
                raise
            raise CollectionError(f"Error getting load progress: {e}") from e
    ...
    async def release_collection(self, collection_name: str, timeout: float | None = None) -> bool:
        """
        ...
        async with collection_lock:
            ...
            try:
                ...
                logger.error(
                    f"[release_collection] Error releasing collection '{collection_name}' "
                    f"(alias={getattr(e, 'using', 'unknown')}): {e}"
                )
                if isinstance(e, CollectionNotFoundError | OperationTimeoutError):
                    raise
                raise CollectionError(f"Error releasing collection: {e}") from e
    ...
    def _release_collection_internal(self, alias: str, collection_name: str) -> None:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to release collection: {e}") from e

    async def drop_collection(
        self, collection_name: str, safe: bool = True, timeout: float | None = None
    ) -> bool:
        """
        ...
        async with collection_lock:
            ...
            if safe:
                ...
                try:
                    ...
                    if description.load_state in [LoadState.LOADED, LoadState.LOADING]:
                        operation = (
                            "loaded" if description.load_state == LoadState.LOADED else "loading"
                        )
                        error_msg = (
            ...
            try:
                ...
                logger.error(
                    f"[drop_collection] Error dropping collection '{collection_name}' "
                    f"(alias={getattr(e, 'using', 'unknown')}): {e}"
                )
                if isinstance(e, CollectionNotFoundError | OperationTimeoutError):
                    raise
                raise CollectionError(f"Error dropping collection: {e}") from e
    ...
    async def insert(
        self, collection_name: str, data: list[list], timeout: float | None = None
    ) -> Any:
        """
        ...
        try:
            ...
            raise CollectionError(f"Failed to insert data: {e}") from e
    ...
    async def get_collection_stats(
        ...
        try:
            ...
            logger.error(
                f"[get_collection_stats] Error getting stats for collection '{collection_name}' "
                f"(alias={getattr(e, 'using', 'unknown')}): {e}"
            )
            if isinstance(e, CollectionNotFoundError | OperationTimeoutError):
                raise
            raise CollectionError(f"Error getting collection stats: {e}") from e
