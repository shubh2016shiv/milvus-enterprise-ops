...
class BackupManager:
    ...
    async def create_backup(
        ...
        try:
            # Acquire lock
            lock = await self._acquire_collection_lock(collection_name)
            ...
            async with lock:
                ...
                if self._config.auto_verify_after_backup:
                    verification = await self.verify_backup(backup_id, collection_name)
    ...
    async def list_backups(
        ...
        try:
            backend = self._get_backend(storage_type)
            backups = await backend.list_backups(collection_name)
    ...
    async def get_backup_info(
        ...
        try:
            backend = self._get_backend(storage_type)
            return await backend.get_backup_metadata(collection_name, backup_id)
        except Exception as e:
    ...
    async def delete_backup(
        ...
        try:
            backend = self._get_backend(storage_type)
            result = await backend.delete_backup(collection_name, backup_id)
    ...
    async def verify_backup(
        ...
        try:
            ...
            is_valid = await backend.verify_backup(collection_name, backup_id)
    ...
    async def apply_retention_policy(
        ...
        try:
            # Get all backups
            backups = await self.list_backups(collection_name, storage_type)
            ...
            if not dry_run and to_delete:
                for backup_version in to_delete:
                    success = await self.delete_backup(
